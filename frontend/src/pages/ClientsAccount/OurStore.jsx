    ...
    <div className="mt-24" id="ourstore">
      ...
      <div className="flex flex-wrap gap-8 mt-10 justify-center mb-20">
        {storeItems.map((item, index) => (
          <div
            key={index}
            className={`relative bg-[#E0EF8F] w-[20%] p-7 ${item.shape} flex items-center justify-center text-center`}
          >
            {/* Image and Name Container */}
            <div className="flex flex-col items-center justify-center text-center">
              <img src={item.img} alt={item.name} className="w-[60%] h-[25vh]" />
            ...
            <div className="absolute right-4 top-1/2 transform -translate-y-1/2">
              <Link to='/buyitem'><FaArrowAltCircleRight className="text-[#0D2F26] text-[35px]" /></Link>
            </div>
          </div>
        ))}
      </div>
